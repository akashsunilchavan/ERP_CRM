...
export const Harvesting = () => {
    ...
    const tableColumns = [
        ...
        { text: 'Room Name', th: { id: 'room', style: { minWidth: '230px', width: '230px' } } },
    ...
    let searchQuery = useMemo(() => {
        getRecords(`harvesting?start_date=${filterStartDate}&end_date=${filterEndDate}&labour=${filterLabours}&supervisor=${filteredSupervisors}&room=${filterRoom}`, setHarvestData);
        return `harvesting?start_date=${filterStartDate}&end_date=${filterEndDate}&labour=${filterLabours}&supervisor=${filteredSupervisors}&room=${filterRoom}`;
                    ...
                    tableBody={() =>
                        harvestData?.data?.map((obj: any, index: number) => (
                            <tr key={obj.id} className="border-b hover:bg-gray-50 transition">
                                ...
                                <td>{obj.room ? obj.room.room_name : '—'}</td>
